--[[
    CustomLib_TopTabs (v4) - with Config & Theme System
    
    Новое:
      - Полная система конфигов (сохранение/загрузка)
      - Автоматическая привязка элементов к флагам
      - Пользовательские темы с сохранением
      - Меню управления темами и конфигами
...
function Library:AutoLoadConfig()
    if not self.AutoLoad then return end
    
    -- Пытаемся загрузить последний использованный конфиг
    local configs = self:GetConfigs()
    if #configs > 0 then
        -- Ищем конфиг с пометкой LastUsed
        for _, name in ipairs(configs) do
            local data = self:LoadConfig(name)
            if data and data._LastUsed then
                self:LoadCurrentConfig(name)
                return
            end
        end
        -- Если не нашли, загружаем первый
        self:LoadCurrentConfig(configs[1])
    end
end

function Library:MarkLastUsed(name)
    local data = self:LoadConfig(name)
    if data then
        data._LastUsed = os.time()
        self:SaveConfig(name, data)
    end
end
...
function Library:LoadTheme(name)
    local path = self.ThemeFolder .. "/" .. name .. ".json"
    if not isfile(path) then return nil end
    local data = readfile(path)
    local themeData = HttpService:JSONDecode(data)
    self:AddTheme(name, themeData)
    return themeData
end
...
function Library:CreateThemeFromCurrent(name)
    local themeData = {
    ...
    return self:SaveTheme(name, themeData)
end
...
function Library:CreateWindow(config)
    ...
    -- Сохраняем ссылку на окно в библиотеке
...
function Library:AddSettingsMenu(window)
    ...
    ThemeManagerSection:AddButton("Save Current Theme", function()
        local name = "MyTheme_" .. os.time()
        self:CreateThemeFromCurrent(name)
    ...
    -- Сбросить настройки
    ConfigSection:AddButton("Reset All Settings", function()
